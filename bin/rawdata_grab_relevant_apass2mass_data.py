#!/usr/bin/env python
"""
    rawdata_grab_relevant_apass2mass_data.py

    Example:
    
    rawdata_grab_relevant_apass2mass_data.py --help

    rawdata_grab_relevant_apass2mass_data.py --inputFile inputFile.csv --outputFile outputFile.csv --verbose 2
...
def main():
    ...
    status = grab_relevant_apass2mass_data(args)
...
def grab_relevant_apass2mass_data(args):
    ...
    if args.verbose > 0:
        print "grab_relevant_apass2mass_data"
    ...
    df.loc[:, 'RA_WRAP'] = df.loc[:, 'RAJ2000_APASS']
    df.loc[:, 'RA']  = df.loc[:, 'RAJ2000_APASS']
    df.loc[:, 'DEC'] = df.loc[:, 'DEJ2000_APASS']
    
    # Trick from S Allam:
    #df.rename(columns={'RAJ2000_APASS':'RA_WRAP'},inplace=True)
    #df.rename(columns={'DEJ2000_APASS':'DEC'},inplace=True)
    #df.rename(columns={'E_RAJ2000_APASS':'RAERR_WRAP'},inplace=True)
    #df.rename(columns={'E_DEJ2000_APASS':'DECERR'},inplace=True)
    #df.rename(columns={'RAJ2000_APASS':'RA_WRAP','DEJ2000_APASS':'DEC', 'E_RAJ2000_APASS':'RAERR_WRAP', 'E_DEJ2000_APASS':'DECERR'},inplace=True)
    ...
    # Sort by RA_WRAP...
    df.sort('RA_WRAP', ascending=True, inplace=True)
